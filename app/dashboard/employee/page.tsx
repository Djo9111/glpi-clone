              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">
                    Besoin logiciel ou bureautique (Word, Excel, accès réseau, etc.)
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">
                    Panne matérielle nécessitant un déplacement technique
                  </p>
                </div>
              </div>
            </div>
      ...
      <aside className="absolute right-0 top-0 h-full w-full sm:w-[480px] bg-white shadow-2xl border-l border-slate-200 p-6 flex flex-col overflow-y-auto">
        ...
        <div className="mb-6 bg-white border border-slate-200 rounded-lg p-4">
            ...
            {steps.map((lbl, i) => (
              <div key={lbl} className="flex flex-col items-center gap-1.5 flex-1">
                <div className={`h-2.5 w-2.5 rounded-full transition-all ${
                  i < step ? "bg-blue-500 scale-110" : i === step ? "bg-blue-400 ring-4 ring-blue-100" : "bg-slate-200"
                }`} />
                <span className={`text-[10px] font-medium ${i <= step ? "text-slate-700" : "text-slate-400"}`}>
                  {lbl}
                </span>
                {i !== steps.length - 1 && (
                  <div className={`absolute h-0.5 w-full mt-1 ${i < step ? "bg-blue-400" : "bg-slate-200"}`} 
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 text-xs text-slate-600 flex items-center gap-1.5">
            ...
            <span>Technicien assigné : {ticket.assignedTo ? `${ticket.assignedTo.prenom} ${ticket.assignedTo.nom}` : "En attente d'attribution"}</span>
          </div>
        </div>
              ...
              {comments.map((c) => (
                <li key={c.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
                    <span className="font-medium text-slate-700">{c.auteur.prenom} {c.auteur.nom}</span>

...
export async function GET(req: Request) {
  ...
  try {
    ...
    console.error("GET /api/admin/materiels:", e);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const created = await prisma.materiel.create({ data: { nom: name } });
    return NextResponse.json(created, { status: 201 });
  } catch (e: any) {
    if (e?.code === "P2002") {
      return NextResponse.json({ error: "Ce matériel existe déjà" }, { status: 409 });
    }
    console.error("POST /api/admin/materiels:", e);
